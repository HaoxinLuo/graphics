...
def main():
    ...
    while(True):
        ...
        cache[1:]=[float(q) if not(cache[0]=='file' or cache[0][0]=='#') else q for q in cache[1:]]
        if cache[0] == '#' or cache[0][0] == '#':
            pass
        ...
        ######################################################
            ...
            if cache[1] not in files:
                files[cache[0]] = '-1'
            t[cache[0]] = str(int(t[cache[0]])+1).zfill(2)
            n = open(cache[0]+t[cache[0]],'w')
            n.write("P3\n "+str(len(pix))+' '+str(len(pix[0]))+"\n 255\n")
            for y in range(len(pix[0])):
                for x in range(len(pix)):
                    n.write(pix[x][y] + ' ')            
